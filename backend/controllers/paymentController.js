...
const paymentController = {
    ...
    // @access Private (Owner, Employee, Customer with ownership check)
    getPaymentsByOrder: async (req, res) => {
        try {
            ...
            const [orderRows] = await pool.query('SELECT customer_id FROM orders WHERE id = ?', [Number(order_id)]);
            if (orderRows.length === 0) {
                return res.status(404).json({ success: false, message: 'Order not found.' });
            }

            if (req.user.role === 'customer' && Number(orderRows[0].customer_id) !== Number(req.user.id)) {
                return res.status(403).json({ success: false, message: 'Access denied: You can only view payments for your own orders.' });
            }
