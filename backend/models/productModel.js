    ...
    getProductById: async (id, includeInactive = false) => {
        let sql = `
            SELECT
                ...
                COALESCE(i.quantity_available, v.quantity, 0) AS quantity_available
            FROM products p
    ...
    updateProduct: async (id, productData, variantsData) => {
        ...
        try {
            ...
            for (const variant of variantsData) {
                ...
                const variantId = varResult.insertId;
                const initialQty = variant.quantity_available !== undefined ? Number(variant.quantity_available) : Number(variant.quantity) || 0;
                await connection.execute(
                    'INSERT INTO inventory (variant_id, quantity_available) VALUES (?, ?)',
                    [variantId, initialQty]
                );
